...
export async function runPlanFlow(
  ...
  try {
    ...
    const intentPrompt = `分析用户需求：${userMessage}`;
    const intentResponse = await llmService.chat([{ role: 'user', content: intentPrompt }]);
    ...
    const planPrompt = `为以下需求生成执行计划：${userMessage}`;
    const planResponse = await llmService.chat([{ role: 'user', content: planPrompt }]);
